...
class AdvancedAnalyzer:
    ...
    def surge_sensitivity_analysis(self, 
                                 city_id: int, 
                                 start_cluster: str, 
                                 start_hour: int, 
                                 work_hours: int, 
                                 start_date: datetime,
                                 surge_multipliers: List[float]) -> pd.DataFrame:
        """
    ...
    def cluster_popularity_analysis(self, city_id: int, hour: int) -> pd.DataFrame:
        ...
        for cluster in nodes:
            ...
            earning_rate = self.optimizer.compute_earning_rate(
                graph, cluster, hour, city_id, datetime(2023, 1, 15)  # Use sample date
            )
    ...
    def export_results_to_json(self, 
                             ...
                             filename: str = "dp_analysis_results.json"):
        """
...
def comprehensive_analysis_example():
    ...
    schedule_results = analyzer.compare_work_schedules(
        city_id, start_cluster, start_date, schedules
    )
    ...
    weekly_results = analyzer.weekly_analysis(
        city_id, start_cluster, 8, 8, start_date
    )
    ...
    print(f"Best day: {weekly_results.loc[weekly_results['total_earnings'].idxmax(), 'day_of_week']} (€{weekly_results['total_earnings'].max():.2f})")
    print(f"Worst day: {weekly_results.loc[weekly_results['total_earnings'].idxmin(), 'day_of_week']} (€{weekly_results['total_earnings'].min():.2f})")
    ...
    # Summary insights
    print("\n\n=== KEY INSIGHTS ===")
    print(f"🏆 Best work schedule: {schedule_results.iloc[0]['schedule']} (€{schedule_results.iloc[0]['total_earnings']:.2f})")
    print(f"📊 Average weekly earnings: €{weekly_avg * 7:.2f} per week")
    print(f"📈 Surge impact: 2x surge pricing increases earnings by {((surge_results[surge_results['surge_multiplier']==2.0]['earnings_vs_baseline'].iloc[0] - 1) * 100):.1f}%")
    print(f"🎯 Best earning cluster: {popularity_results.iloc[0]['cluster']} (€{popularity_results.iloc[0]['earning_rate']:.2f}/h)")
