...
def display_single_rate(
  ...
  if verbose:
    ...
    for dest in nodes:
      ...
      if prob > 0.01:  # Only show significant probabilities
        if graph.has_edge(cluster, dest):
          ...
          print(
            f"     Contributes: €{contribution_fare:.2f} to fare, {contribution_time:.1f}min to time"
          )
    ...
    print(
      f"  Earning rate: €{expected_fare:.2f} ÷ ({total_time:.1f}/60) = €{earning_rate:.2f}/hour"
    )


def display_all_clusters(optimizer: MobilityOptimizer, city_id: int, hour: int, date: datetime):
...
Examples:
  ...
  parser.add_argument("--date", type=parse_date, required=True, help="Date in YYYY-MM-DD format")

  # Analysis type (mutually exclusive)
  analysis_group = parser.add_mutually_exclusive_group(required=True)
  analysis_group.add_argument("--cluster", type=str, help="Check specific cluster (e.g., c_3_2)")
  analysis_group.add_argument(
    "--all-clusters", action="store_true", help="Check all clusters at specific hour"
  )
  analysis_group.add_argument(
    "--compare-hours", action="store_true", help="Compare cluster across different hours"
  )
